...
The app must not fall back to direct WebSocket or SSH-forwarded WebSocket when SSH stdio fails.
Configuration, authentication, and connection failures should surface the root cause so the demo
environment can be fixed.

For simulator relaunch, explicit non-secret `AWAY_*` SSH stdio settings may be persisted in user
defaults, while SSH passwords and private-key material must be persisted in the app keychain. Stale
WebSocket demo settings should fail with an explicit reset/relaunch message instead of selecting a
WebSocket transport.
